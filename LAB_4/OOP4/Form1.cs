...
namespace OOP2
{
    public partial class Form1 : Form
    {
        ...
        private void serbtn_Click(object sender, EventArgs e)
        {
            ...
            serializer.Serialize(ReadyInbox, ReadyEvent, ReadyProject, ReadySub, array);
        ...
        {
            var serializer = new Serializer();
            serializer.Deserialize(ReadyInbox, ReadyEvent, ReadyProject, ReadySub, array);
            
            for (int i = 0; i < array.listIn.Count; i++)
            {
                AddInfoInboxGrid(array.listIn[i].nameItem);
                ReadyInbox.Add(array.listIn[i].nameItem);
            }
            for (int i = 0; i < array.listEv.Count; i++)
            {
                ...
                ReadyEvent.Add(array.listEv[i].nameItem, array.listEv[i].dateItem, array.listEv[i].timeItem);
            }
            for (int i = 0; i < array.listProj.Count; i++)
            {
                AddInfoProject(array.listProj[i].nameItem);
                ReadyProject.Add(array.listProj[i].nameItem);
            }
            for (int i = 0; i < array.listSub.Count; i++)
            {
                for (int ind = 0; ind <= treeView1.Nodes.Count - 1; ind++)
                {
                    if (treeView1.Nodes[ind].Text == array.listSub[i].prname)
                    {
                        ...
                        ReadySub.Add(treeView1.Nodes[ind].Text, array.listSub[i].prname);
                    }
